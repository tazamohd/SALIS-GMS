...
- **Feature #5 - Real-Time Parts Availability Tracker**: Multi-supplier inventory sync with cached availability data. Database table `supplier_parts_availability` tracks quantity, price, lead time, and sync timestamps per garage/supplier/part. Comprehensive CRUD API routes (GET/search, POST/sync, GET/:id, POST, PATCH/:id, DELETE/:id) with full tenant isolation (garage-scoped). UI at `/parts-availability` features search interface, mock sync button, responsive card grid with status badges (In Stock/Low Stock/Out of Stock), and real-time updates via TanStack Query. All endpoints secured with authentication and Zod validation. **Backlog**: TecDoc API integration for live external data, automated background sync with cron jobs, pagination, advanced search with spareParts join, seed data for testing.
