...
**Real-Time Features**: WebSocket server (`/ws/chat`) for in-app chat support, live notifications, and call center real-time updates. Session-based authentication with cryptographic cookie validation. Garage-scoped broadcasting for tenant isolation.
...
- **Feature #5 - Real-Time Parts Availability Tracker (COMPLETE)**: Multi-supplier inventory sync with cached availability data. Database table `supplier_parts_availability` tracks quantity, price, lead time, and sync timestamps per garage/supplier/part. Comprehensive CRUD API routes (GET/search, POST/sync, GET/:id, POST, PATCH/:id, DELETE/:id) with full tenant isolation (garage-scoped). UI at `/parts-availability` features search interface, mock sync button, responsive card grid with status badges (In Stock/Low Stock/Out of Stock), and real-time updates via TanStack Query. All endpoints secured with authentication and Zod validation. Security hardening applied: all CRUD operations properly garage-scoped with combined and() clauses preventing cross-tenant data leaks. **Backlog**: TecDoc API integration for live external data, automated background sync with cron jobs, pagination, advanced search with spareParts join, seed data for testing.
- **Feature #6 - Smart Job Assignment System (COMPLETE)**: AI-powered technician recommendation engine using OpenAI GPT-5 for optimal job assignments. Three database tables: `assignment_rules` (configurable assignment logic), `assignment_history` (audit trail), `ai_assignment_recommendations` (ML predictions with confidence scores). Storage layer includes 10 methods with full tenant isolation: CRUD for rules, recommendation persistence, assignment history tracking, and domain helpers (`getJobCardWithContext`, `getTechniciansWithLoad`, `assignTechnicianToJob` with transactional safety). AI service in `server/services/assignmentAI.ts` analyzes job requirements, technician skills/workload/experience against configurable rules, enriches recommendations with canonical technician metadata (name, email, level, active job count), and persists with IDs for tracking acceptance. Six API routes (POST /recommend/:jobCardId, POST /assign, GET /history/:jobCardId, GET/POST/DELETE /rules) with garage-scoped validation and Zod schemas. UI at `/smart-assignment` displays top 3 AI recommendations with confidence badges, matched skills, workload/availability indicators, manual assignment override, and tracks aiRecommendationId to update wasAccepted flag on acceptance. Architect-approved: complete data flow verified, no security issues, production ready.
- **Feature #7 - Call Center Module (COMPLETE)**: Production-ready call center infrastructure for customer interaction management with real-time updates via WebSocket. Eight database tables: `call_queues` (routing groups), `call_queue_members` (agent assignments with unique constraint on queueId+agentUserId), `call_sessions` (call records with Twilio SID tracking), `call_events` (audit trail), `call_notes` (agent annotations), `call_recordings` (Twilio recording URLs), `call_disposition_codes` (outcome categorization), `agent_performance_snapshots` (metrics aggregation). Storage layer includes 33 methods with strict garage-scoped tenant isolation: CRUD for queues/members/sessions/notes/recordings/codes/performance, domain helpers (`getCallQueueWithMembers`, `listQueueMembers`), and transactional `assignCallToAgent` with automatic event logging. Twenty-seven RESTful API routes with security-hardened PATCH endpoints that strip privileged fields (garageId, queueId, customerId, vehicleId, assignedAgentId) preventing cross-tenant data tampering. All routes use `isAuthenticated` middleware, Zod validation on POST requests, garage-scoped queries, and rate limiting (100 req/15min per IP via `express-rate-limit` on all 15 mutation endpoints) for DoS protection. Real-time WebSocket integration extends existing chat infrastructure: broadcasts garage-scoped events (`call-center.session.updated`, `call-center.queue.updated`) on all mutations, frontend hook (`useCallCenterWebSocket`) validates all payloads with Zod schemas (`selectCallSessionSchema`, `selectCallQueueSchema`) before cache updates preventing poisoning, updates TanStack Query caches for live UI updates with 5-second polling fallback for resilience, and terminates unauthenticated sockets (4003) attempting to join call-center. Dashboard UI at `/call-center` displays Live Calls, Queues, and History tabs with real-time metrics (active calls, completed today, wait time), status badges (Ringing/In Progress/Completed), and dynamic filtering. Security hardening complete: WebSocket auth bypass fixed, cache poisoning prevented, DoS protection implemented. All components architect-approved: schema, storage, API routes, WebSocket integration, security measures, and UI all verified with zero security vulnerabilities, production ready. **Backlog**: Twilio Voice SDK integration for capability tokens/webhooks/outbound calls deferred (requires TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, TWILIO_PHONE_NUMBER credentials).
