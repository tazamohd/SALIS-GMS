...
export function Estimates() {
  ...
  const getStatusColor = (status: string) => {
    const colors: Record<string, string> = {
      ...
      expired: "bg-neon-blue/20/30 text-gray-600",
    };
        ...
        <CardContent className="p-6">
          {isLoading ? (
            <div className="text-center py-12">
              ...
              <p className="text-soft-white/60">Loading estimates...</p>
            </div>
              ...
              {filteredEstimates.map((estimate) => {
                ...
                return (
                  <EstimateDetailsDialog key={estimate.id} estimateId={estimate.id}>
                    <Card className="bg-dark-navy border-neon-blue/30 p-4 cursor-pointer hover:shadow-md transition-shadow" data-testid={`card-estimate-${estimate.id}`}>
                      <div className="flex items-center justify-between">
                        <div className="flex-1">
                          <div className="flex items-center gap-3">
                            <FileText className="w-5 h-5 text-soft-white/50" />
                            <div>
                              <p className="font-['Poppins',Helvetica] font-semibold text-sm text-soft-white" data-testid={`text-estimate-number-${estimate.id}`}>
                                {estimate.estimateNumber}
                              </p>
                              <p className="font-['Poppins',Helvetica] font-normal text-xs text-soft-white/60" data-testid={`text-customer-${estimate.id}`}>
                        ...
                        <div className="flex items-center gap-4">
                          <div className="text-right">
                            <p className="font-['Poppins',Helvetica] font-bold text-sm text-soft-white" data-testid={`text-amount-${estimate.id}`}>
                              ${estimate.totalAmount}
                            </p>
                            {estimate.validUntil && (
                              <p className="font-['Poppins',Helvetica] font-normal text-xs text-soft-white/60">
