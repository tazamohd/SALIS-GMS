...
export default function TechnicianDashboard() {
  ...
  const getPriorityBadge = (priority: string) => {
    const variants: Record<string, { className: string }> = {
      ...
      high: { className: "bg-orange-100 dark:bg-orange-900 text-orange-700 dark:text-orange-300" },
      medium: { className: "bg-yellow-100 dark:bg-yellow-900 text-yellow-700 dark:text-yellow-300" },
      low: { className: "bg-gray-100 dark:bg-gray-900 text-gray-700 dark:text-gray-300" },
    };
  ...
  return (
    <div className="space-y-6">
        ...
        <CardContent>
          {todayJobs.length === 0 ? (
            <div className="text-center py-12">
              <Clock className="h-12 w-12 mx-auto text-gray-400 dark:text-gray-600 mb-3" />
              <p className="text-gray-600 dark:text-gray-400">No jobs scheduled for today</p>
            </div>
          ) : (
            <div className="space-y-4">
              {todayJobs.map((job) => (
                <div
                  key={job.id}
                  className="flex items-center justify-between p-4 bg-gray-50 dark:bg-salis-gray rounded-lg"
      ...
      <Card className="bg-white dark:bg-salis-gray-dark border-gray-200 dark:border-salis-gray">
        <CardHeader>
          <CardTitle className="text-gray-900 dark:text-white">Active Jobs</CardTitle>
          <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
