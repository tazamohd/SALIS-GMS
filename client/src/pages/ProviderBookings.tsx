...
export default function ProviderBookings() {
  ...
  const act = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["/api/provider/bookings"] }); toast({ title: t("providerBookings.bookingUpdated", "Booking updated") }); },
  ...
  const actOrder = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["/api/provider/orders"] }); toast({ title: t("providerBookings.orderUpdated", "Order updated") }); },
  });
  ...
  const respondQuote = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["/api/provider/quotes"] }); toast({ title: t("providerBookings.quoteSent", "Quote sent") }); },
    onError: (e: Error) => toast({ title: t("providerBookings.couldNotRespond", "Could not respond"), description: e.message, variant: "destructive" }),
  });

  return (
    <div className="p-6 space-y-6 bg-[#F8FAFC] dark:bg-[#0E1117] min-h-screen">
      <h1 className="text-2xl font-bold text-[#0B1F3B] dark:text-white flex items-center gap-2"><CalendarClock className="h-6 w-6 text-[#0A5ED7]" /> {t("providerBookings.title", "Incoming Bookings")}</h1>
      <Card className="border-[#E2E8F0] dark:border-[#232A36]">
        <CardHeader><CardTitle className="text-base">{t("providerBookings.marketplaceRequests", "Marketplace requests")}</CardTitle></CardHeader>
        <CardContent>
          {bookings.isLoading ? <p className="text-sm text-[#64748B]">{t("common.loading", "Loading…")}</p>
          : (bookings.data?.length ?? 0) === 0 ? <p className="text-sm text-[#64748B]" data-testid="no-provider-bookings">{t("providerBookings.noBookings", "No bookings yet.")}</p>
          : (
                ...
                <TableBody>
                  {bookings.data!.map((b) => (
                    <TableRow key={b.id} data-testid={`pbooking-${b.id}`}>
                      <TableCell className="font-medium">{b.serviceName ?? t("providerBookings.general", "General")}</TableCell>
                      ...
                      <TableCell className="text-end space-x-1">
                        ...
                        {b.status === "accepted" && (
                          <Button size="sm" data-testid={`complete-${b.id}`} onClick={() => act.mutate({ id: b.id, status: "completed" })} className="bg-blue-600 hover:bg-blue-700 text-white h-8">{t("providerBookings.markComplete", "Mark complete")}</Button>
                        )}
                      </TableCell>
                    </TableRow>
                  ))}
      ...
      {(orders.data?.length ?? 0) > 0 && (
        <Card className="border-[#E2E8F0] dark:border-[#232A36]">
          <CardHeader><CardTitle className="text-base">{t("providerBookings.productOrders", "Product orders")}</CardTitle></CardHeader>
            ...
            {orders.data!.map((o: any) => (
              <div key={o.id} className="p-3 rounded-lg border border-[#E2E8F0] dark:border-[#232A36] flex items-start justify-between gap-3" data-testid={`porder-${o.id}`}>
                ...
                <div className="flex gap-1">
                  {o.status === "pending" && (
                    <>
                      <Button size="sm" className="bg-emerald-600 hover:bg-emerald-700 text-white h-8" onClick={() => actOrder.mutate({ id: o.id, status: "confirmed" })} data-testid={`confirm-order-${o.id}`}>{t("providerBookings.confirm", "Confirm")}</Button>
                      <Button size="sm" variant="outline" className="h-8" onClick={() => actOrder.mutate({ id: o.id, status: "declined" })} data-testid={`decline-order-${o.id}`}>{t("providerBookings.decline", "Decline")}</Button>
                    </>
                  )}
                  {o.status === "confirmed" && (
                    <Button size="sm" className="bg-blue-600 hover:bg-blue-700 text-white h-8" onClick={() => actOrder.mutate({ id: o.id, status: "fulfilled" })} data-testid={`fulfill-order-${o.id}`}>{t("providerBookings.markFulfilled", "Mark fulfilled")}</Button>
                  )}
                </div>
              </div>
            ))}
      ...
      {(quotes.data?.length ?? 0) > 0 && (
        <Card className="border-[#E2E8F0] dark:border-[#232A36]">
          <CardHeader><CardTitle className="text-base">{t("providerBookings.insuranceQuoteRequests", "Insurance quote requests")}</CardTitle></CardHeader>
          <CardContent className="space-y-3">
  ...
  return (
    <div className="p-3 rounded-lg border border-[#E2E8F0] dark:border-[#232A36] flex items-start justify-between gap-3 flex-wrap" data-testid={`pquote-${quote.id}`}>
      <div>
        ...
        {quote.quotedPremium && <div className="text-xs text-emerald-600">{t("providerBookings.quoted", "Quoted")}: {quote.quotedPremium} {quote.currency}</div>}
      </div>
      {quote.status === "pending" && (
        <div className="flex items-center gap-1">
          <input
            type="number" min={1} placeholder={t("providerBookings.premiumSar", "Premium (SAR)")} value={premium}
          ...
          />
          <Button size="sm" className="bg-emerald-600 hover:bg-emerald-700 text-white h-8" disabled={!(Number(premium) > 0)} onClick={() => onRespond("quoted", premium)} data-testid={`send-quote-${quote.id}`}>{t("providerBookings.sendQuote", "Send quote")}</Button>
          <Button size="sm" variant="outline" className="h-8" onClick={() => onRespond("declined")} data-testid={`decline-quote-${quote.id}`}>{t("providerBookings.decline", "Decline")}</Button>
        </div>
      )}
    </div>
  );
