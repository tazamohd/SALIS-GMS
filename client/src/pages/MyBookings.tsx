...
export default function MyBookings() {
  ...
  const cancel = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["/api/my/bookings"] }); toast({ title: t("myBookings.bookingCancelled", "Booking cancelled") }); },
    onError: (e: Error) => toast({ title: t("myBookings.couldNotCancel", "Could not cancel"), description: e.message, variant: "destructive" }),
  });
  ...
  const cancelOrder = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["/api/my/orders"] }); toast({ title: t("myBookings.orderCancelled", "Order cancelled") }); },
  });
  ...
  const decideQuote = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["/api/my/quotes"] }); toast({ title: t("common.updated", "Updated") }); },
    onError: (e: Error) => toast({ title: t("myBookings.couldNotUpdate", "Could not update"), description: e.message, variant: "destructive" }),
  });
    ...
    <div className="min-h-screen p-6 bg-[#F8FAFC] dark:bg-[#0E1117]">
      <div className="max-w-4xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold text-[#0B1F3B] dark:text-white flex items-center gap-2"><CalendarClock className="h-6 w-6 text-[#0A5ED7]" /> {t("myBookings.title", "My Bookings")}</h1>
          <Link href="/marketplace"><Button variant="outline">{t("myBookings.findProvider", "Find a provider")}</Button></Link>
        </div>
            ...
            <CardContent className="p-4 space-y-2">
              {unread.slice(0, 5).map((n) => (
                <div key={n.id} className="flex items-start justify-between gap-3" data-testid={`notif-${n.id}`}>
                  ...
                  <Button size="sm" variant="ghost" className="text-xs" onClick={() => markRead.mutate(n.id)} data-testid={`notif-read-${n.id}`}>{t("myBookings.dismiss", "Dismiss")}</Button>
                </div>
              ))}
            </CardContent>
          </Card>
        ...
        {bookings.isLoading ? <p className="text-sm text-[#64748B]">{t("common.loading", "Loading…")}</p>
        : (bookings.data?.length ?? 0) === 0 ? (
          <p className="text-sm text-[#64748B]" data-testid="no-bookings">{t("myBookings.noBookings", "No bookings yet.")} <Link href="/marketplace" className="text-[#0A5ED7] dark:text-[#0BB3FF] hover:underline">{t("myBookings.browseProviders", "Browse providers")}</Link>.</p>
        ) : (
                ...
                <CardContent className="p-4 flex items-start justify-between gap-3">
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      <span className="font-semibold text-[#0B1F3B] dark:text-white">{b.serviceName ?? t("myBookings.serviceRequest", "Service request")}</span>
                    ...
                    {b.preferredDate && <div className="text-xs text-[#64748B]">{t("myBookings.preferred", "Preferred")}: {new Date(b.preferredDate).toLocaleString()}</div>}
                    {b.providerNotes && <div className="text-xs text-[#0A5ED7] dark:text-[#0BB3FF]">{t("myBookings.provider", "Provider")}: {b.providerNotes}</div>}
                  </div>
                  {(b.status === "requested" || b.status === "accepted") && (
                    <Button size="sm" variant="outline" onClick={() => cancel.mutate(b.id)} data-testid={`cancel-${b.id}`}>{t("common.cancel", "Cancel")}</Button>
                  )}
                </CardContent>
              </Card>
            ))}
        ...
        {(orders.data?.length ?? 0) > 0 && (
          <div className="space-y-3">
            <h2 className="text-sm font-semibold text-[#64748B] uppercase tracking-wide">{t("myBookings.myOrders", "My Orders")}</h2>
              ...
              <Card key={o.id} className="border-[#E2E8F0] dark:border-[#232A36]" data-testid={`order-${o.id}`}>
                <CardContent className="p-4 flex items-start justify-between gap-3">
                  <div className="space-y-1">
                    ...
                    {o.providerNotes && <div className="text-xs text-[#0A5ED7] dark:text-[#0BB3FF]">{t("myBookings.store", "Store")}: {o.providerNotes}</div>}
                  </div>
                  {(o.status === "pending" || o.status === "confirmed") && (
                    <Button size="sm" variant="outline" onClick={() => cancelOrder.mutate(o.id)} data-testid={`cancel-order-${o.id}`}>{t("common.cancel", "Cancel")}</Button>
                  )}
                </CardContent>
              </Card>
            ))}
        ...
        {(quotes.data?.length ?? 0) > 0 && (
          <div className="space-y-3">
            <h2 className="text-sm font-semibold text-[#64748B] uppercase tracking-wide">{t("myBookings.myInsuranceQuotes", "My Insurance Quotes")}</h2>
            {quotes.data!.map((qt: any) => (
              <Card key={qt.id} className="border-[#E2E8F0] dark:border-[#232A36]" data-testid={`quote-${qt.id}`}>
                <CardContent className="p-4 flex items-start justify-between gap-3">
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      <span className="font-semibold text-[#0B1F3B] dark:text-white">{qt.planName ?? t("myBookings.insuranceQuote", "Insurance quote")}</span>
                    ...
                    {qt.quotedPremium && <div className="text-xs text-emerald-600 font-medium">{t("myBookings.premium", "Premium")}: {qt.quotedPremium} {qt.currency}</div>}
                    {qt.quoteNotes && <div className="text-xs text-[#0A5ED7] dark:text-[#0BB3FF]">{qt.quoteNotes}</div>}
                  </div>
                  <div className="flex gap-1">
                    {qt.status === "quoted" && (
                      <Button size="sm" className="bg-emerald-600 hover:bg-emerald-700 text-white" onClick={() => decideQuote.mutate({ id: qt.id, d: "accept" })} data-testid={`accept-quote-${qt.id}`}>{t("myBookings.accept", "Accept")}</Button>
                    )}
                    {(qt.status === "pending" || qt.status === "quoted") && (
                      <Button size="sm" variant="outline" onClick={() => decideQuote.mutate({ id: qt.id, d: "cancel" })} data-testid={`cancel-quote-${qt.id}`}>{t("common.cancel", "Cancel")}</Button>
                    )}
                  </div>
                </CardContent>
              </Card>
