...
interface ForecastResponse {
  weeklyForecast: Array<{ week: string; predicted: number; historical: number | null; confidence: number }>;
  partsDemand: Array<{ part: string; current: number; forecasted: number; reorderPoint: number }>;
  accuracy: number;
  peakDay: string;
  partsReorderAlerts: number;
...
export default function PredictiveDemandForecasting() {
  ...
  const { data: forecast, isLoading } = useQuery<ForecastResponse>({
    queryKey: [`/api/forecasting/demand?period=${forecastPeriod}`],
  });

  const demandForecast = forecast?.weeklyForecast ?? [];
  const partsDemand = forecast?.partsDemand ?? [];
  const reorderItems = partsDemand.filter(p => p.current < p.reorderPoint);

  const metrics = [
    {
      label: t('demandForecasting.forecastAccuracy', 'Forecast Accuracy'),
      value: forecast ? `${forecast.accuracy}%` : "--",
      icon: TrendingUp,
      color: "text-green-500",
    },
    {
      label: t('demandForecasting.predictedPeakDay', 'Predicted Peak Day'),
      value: forecast?.peakDay ?? "--",
      icon: Calendar,
      color: "text-[#0A5ED7]",
    },
    {
      label: t('demandForecasting.partsReorderAlerts', 'Parts Reorder Alerts'),
      value: forecast ? String(forecast.partsReorderAlerts) : "--",
      icon: Package,
      color: "text-[#F97316]",
    },
    {
      label: t('demandForecasting.avgDailyAppointments', 'Avg Daily Appointments'),
      value: forecast ? String(forecast.avgDailyAppointments) : "--",
      icon: Users,
  ...
  return (
    <DashboardPage
      ...
      description={t('demandForecasting.description', 'Statistical forecasting for service demand and inventory planning')}
      icon={TrendingUp}
      metrics={metrics}
    >
      <div className="mb-6">
          ...
          <CardContent>
            {demandForecast.length === 0 ? (
              <p className="text-sm text-[#64748B] py-16 text-center">
                {isLoading ? t('common.loading', 'Loading...') : t('demandForecasting.noDemandData', 'Not enough activity yet to forecast.')}
              </p>
            ) : (
              <ResponsiveContainer width="100%" height={300}>
                <LineChart data={demandForecast}>
                  ...
                  <Tooltip />
                  <Legend />
                  <Line type="monotone" dataKey="historical" stroke="#0A5ED7" name={t('demandForecasting.historical', 'Historical')} strokeWidth={2} />
                  <Line type="monotone" dataKey="predicted" stroke="#0BB3FF" name={t('demandForecasting.predicted', 'Predicted')} strokeWidth={2} strokeDasharray="5 5" />
                </LineChart>
              </ResponsiveContainer>
            )}
          </CardContent>
        ...
        <Card className="bg-white dark:bg-[#151A23] border-[#E2E8F0] dark:border-[#232A36]">
          ...
          <CardContent>
            {partsDemand.length === 0 ? (
              <p className="text-sm text-[#64748B] py-16 text-center">
                {isLoading ? t('common.loading', 'Loading...') : t('demandForecasting.noPartsData', 'No parts inventory tracked yet.')}
              </p>
            ) : (
              <ResponsiveContainer width="100%" height={300}>
      ...
      <Card className="mb-6 bg-white dark:bg-[#151A23] border-[#E2E8F0] dark:border-[#232A36]">
        <CardHeader>
          ...
          <CardDescription className="text-[#64748B]">
            {t('demandForecasting.reorderRecommendationsDesc', 'Parts requiring attention based on forecasted demand')}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-3">
            {reorderItems.map((part, idx) => (
              <div key={`${part.part}-${idx}`} className="p-4 border border-[#F97316]/30 bg-[#F97316]/5 dark:bg-[#F97316]/10 rounded-lg" data-testid={`reorder-alert-${idx}`}>
                <div className="flex items-center justify-between">
                  <div>
                    <p className="font-medium text-[#0B1F3B] dark:text-white">{part.part}</p>
                    <p className="text-sm text-[#64748B]">
                      {t('demandForecasting.current', 'Current')}: {part.current} {t('demandForecasting.units', 'units')} •{' '}
                      {t('demandForecasting.forecastedNeedLabel', 'Forecasted need')}: {part.forecasted} {t('demandForecasting.units', 'units')}
                    </p>
                  </div>
                  <Badge className="bg-[#F97316] text-white hover:bg-[#F97316]/90">
                    {t('demandForecasting.reorder', 'Reorder')} {Math.max(0, part.forecasted - part.current)} {t('demandForecasting.units', 'units')}
                  </Badge>
                </div>
              </div>
            ))}
            {reorderItems.length === 0 && (
              <p className="text-center text-[#64748B] py-4">
                {partsDemand.length === 0
                  ? (isLoading ? t('common.loading', 'Loading...') : t('demandForecasting.noPartsTracked', 'No parts inventory tracked yet.'))
      ...
      <Card className="bg-white dark:bg-[#151A23] border-[#E2E8F0] dark:border-[#232A36]">
        <CardHeader>
          ...
          <CardDescription className="text-[#64748B]">{t('demandForecasting.forecastConfidenceLevelsDesc', 'Prediction confidence by forecast week')}</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
            {demandForecast.length === 0 ? (
              <p className="text-sm text-[#64748B] text-center py-4">{t('common.noData', 'No data')}</p>
            ) : (
