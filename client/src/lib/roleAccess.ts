...
export const roleNavAccess: Record<UserRole, string[]> = {
  ...
  // Business Intelligence' groups (audit FE-H1). Per-item role gating is still
  // enforced by filterNavigationByAccess (pass 1), so listing a group title
  // here only lets through the items that pass already permits.
  MANAGER: [
