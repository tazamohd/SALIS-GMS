...
export const purchaseTasks = pgTable("purchase_tasks", {
  ...
  title: varchar("title", { length: 255 }).notNull(),
  description: text("description"),
  sourceType: varchar("source_type", { length: 50 }).notNull(), // "procurement" | "store_keeper"
...
export const purchaseTaskParts = pgTable("purchase_task_parts", {
  id: uuid("id")
    .primaryKey()
    .default(sql`gen_random_uuid()`),
  taskId: uuid("task_id")
...
export const quotationRequests = pgTable("quotation_requests", {
  ...
  garageId: uuid("garage_id")
    .notNull()
    .references(() => garages.id),
  taskId: uuid("task_id").references(() => purchaseTasks.id),
  title: varchar("title", { length: 255 }).notNull(),
  status: varchar("status", { length: 20 }).notNull().default("draft"), // "draft" | "sent" | "received" | "selected" | "cancelled"
  storeLocation: varchar("store_location", { length: 255 }),
  dueDate: timestamp("due_date"),
  selectedQuotationId: uuid("selected_quotation_id"),
  createdBy: varchar("created_by")
    .notNull()
    .references(() => users.id),
  createdAt: timestamp("created_at").defaultNow(),
...
export const supplierQuotations = pgTable("supplier_quotations", {
  id: uuid("id")
    .primaryKey()
    .default(sql`gen_random_uuid()`),
  quotationRequestId: uuid("quotation_request_id")
    .notNull()
    .references(() => quotationRequests.id, { onDelete: "cascade" }),
  supplierId: uuid("supplier_id")
    .notNull()
    .references(() => suppliers.id),
...
export const quotationItems = pgTable("quotation_items", {
  ...
  quantity: integer("quantity").notNull(),
  unitPrice: decimal("unit_price", { precision: 10, scale: 2 }).notNull(),
  availability: varchar("availability", { length: 30 }).default("in_stock"), // "in_stock" | "low_stock" | "out_of_stock" | "on_order"
...
export const supplierPayments = pgTable("supplier_payments", {
  ...
  supplierId: uuid("supplier_id")
    .notNull()
    .references(() => suppliers.id),
  supplierBank: varchar("supplier_bank", { length: 100 }),
  supplierIban: varchar("supplier_iban", { length: 50 }),
  invoiceNumber: varchar("invoice_number", { length: 100 }),
  invoiceDate: timestamp("invoice_date"),
  amount: decimal("amount", { precision: 10, scale: 2 }).notNull(),
  currency: varchar("currency", { length: 10 }).default("SAR"),
  status: varchar("status", { length: 20 }).notNull().default("pending"), // "pending" | "approved" | "processing" | "paid" | "overdue" | "cancelled"
  dueDate: timestamp("due_date"),
...
export const deliveries = pgTable("deliveries", {
  ...
  status: varchar("status", { length: 20 }).notNull().default("pending"), // "pending" | "dispatched" | "in_transit" | "arrived" | "delivered" | "delayed"
...
export const deliveryTimeline = pgTable("delivery_timeline", {
  id: uuid("id")
    .primaryKey()
    .default(sql`gen_random_uuid()`),
  deliveryId: uuid("delivery_id")
...
// Purchase Agent - Live Delivery Tracking
export const liveDeliveryStatuses = pgTable("live_delivery_statuses", {
  id: uuid("id")
    .primaryKey()
    .default(sql`gen_random_uuid()`),
  deliveryId: uuid("delivery_id")
  ...
  destinationGarage: varchar("destination_garage", { length: 255 }),
  destinationAddress: text("destination_address"),
  storeKeeperName: varchar("store_keeper_name", { length: 255 }),
  storeKeeperPhone: varchar("store_keeper_phone", { length: 30 }),
  driverName: varchar("driver_name", { length: 255 }),
  driverPhone: varchar("driver_phone", { length: 30 }),
  driverPhoto: varchar("driver_photo", { length: 500 }),
  vehicleNumber: varchar("vehicle_number", { length: 50 }),
  currentStage: varchar("current_stage", { length: 30 }).default("confirmed"), // "confirmed" | "picked_up" | "in_transit" | "arriving" | "delivered"
  estimatedArrival: varchar("estimated_arrival", { length: 50 }),
  stages: jsonb("stages").default([]),
  liveUpdates: jsonb("live_updates").default([]),
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
});
...
export const insertPurchaseTaskSchema = createInsertSchema(purchaseTasks).omit({ id: true, createdAt: true, updatedAt: true });

export type PurchaseTaskPart = typeof purchaseTaskParts.$inferSelect;
export type InsertPurchaseTaskPart = typeof purchaseTaskParts.$inferInsert;
export const insertPurchaseTaskPartSchema = createInsertSchema(purchaseTaskParts).omit({ id: true, createdAt: true });
...
export const insertQuotationRequestSchema = createInsertSchema(quotationRequests).omit({ id: true, createdAt: true, updatedAt: true });
...
export const insertSupplierPaymentSchema = createInsertSchema(supplierPayments).omit({ id: true, createdAt: true, updatedAt: true });
...
export type DeliveryTimelineEvent = typeof deliveryTimeline.$inferSelect;
export type InsertDeliveryTimelineEvent = typeof deliveryTimeline.$inferInsert;
export const insertDeliveryTimelineEventSchema = createInsertSchema(deliveryTimeline).omit({ id: true, createdAt: true });
