...
|------|--------|-----------|
| `customers.routes.ts` | Customer CRUD, notes | 8 |
| `vehicles.routes.ts` | Vehicle CRUD, service history, catalogs, VIN decode | 22 |
| `jobcards.routes.ts` | Job card lifecycle, tasks, parts, ETA, tracking events | 17 |
| `technicians.routes.ts` | Technician profiles, time-clock | 9 |
| `inventory.routes.ts` | Spare parts, inventory records | 8 |
| `invoices.routes.ts` | Invoices, payments, refunds, tax calculation | 10 |
| `scheduling.routes.ts` | Appointments, availability, calendar, time slots | 11 |
...
| `emailService.ts` | Google Gmail API integration |
...
Defines all PostgreSQL tables using Drizzle ORM column builders. Also exports Zod insert/select schemas generated by `drizzle-zod`. This is the single source of truth for data shapes across client, server, and migrations. The schema covers 320+ tables spanning all business domains.
...
`config.ts` initializes i18next with browser language detector and Arabic (`ar`) locale file. `locales/ar.json` contains all Arabic UI strings. English strings are inline in components (default locale).
