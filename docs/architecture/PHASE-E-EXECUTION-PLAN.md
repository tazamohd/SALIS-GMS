...
6. crm ✅ (customer-360 list/detail, segments, loyalty summary + award-points, retention analytics, campaigns — raw-SQL read model moved wholesale into a `crm` module: repository owns every `db.execute(sql…)`, service owns the tier/segment/loyalty/retention math, controller keeps the garage-required 403 and graceful-degradation defaults) / insurance / fleet / marketplace
