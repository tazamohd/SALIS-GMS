...
const canCreateInvoice = await hasPermission(
  userId,
  'invoices',
  'create',
  garageId
...
app.get('/admin/users', 
  requireRole(['System Administrator', 'HR Manager']),
  async (req, res) => {
    // Handler code
  }
);
...
### Finance
- leila.senger@salisauto.com (Finance Manager)
- edgar.heaney@salisauto.com (Accountant)

### Customer Service
- ethelyn.leffler@salisauto.com (Call Center Agent)
- chelsey.dicki-little@salisauto.com (Customer Service Rep)
- jason.jacobson@salisauto.com (Receptionist)

### Other Departments
- hugh.rohan@salisauto.com (HR Manager)
- samson.franecki@salisauto.com (Marketing Manager)
- xavier.pollich-bahringer@salisauto.com (Warehouse Manager)
- evalyn.shanahan@salisauto.com (Quality Control Inspector)

## Implementation Files

- **server/rbac-config.ts** - Role and permission definitions
- **server/rbac-middleware.ts** - Express middleware for access control
- **server/seed-rbac.ts** - Database seeding script for roles and permissions
- **server/create-all-roles.ts** - Script to create all standard roles
- **server/create-staff-users.ts** - Script to populate staff users
- **server/assign-user-roles.ts** - Script to assign roles to existing users
- **shared/schema.ts** - Database schema definitions

## Best Practices

### Security
- Always use role-based checks on both frontend and backend
- Never trust client-side role checks alone
- Implement permission overrides carefully with audit trails
- Use temporary permissions with expiration dates for sensitive access

### Performance
- Cache user permissions in session
- Minimize database queries with eager loading
- Use permission checks at route level, not per-data-item

### Maintenance
- Review and update role permissions quarterly
- Audit permission overrides regularly
...
1. Define role in `server/rbac-config.ts`:
```typescript
NEW_ROLE: {
  name: 'New Role Name',
  scope: 'branch', // or 'garage', 'system', 'franchise'
  isSystemRole: false,
...
1. Add to `RESOURCES` in `server/rbac-config.ts`:
```typescript
new_feature: { 
...
- Check `server/rbac-config.ts` for role definitions
- Consult `server/rbac-middleware.ts` for implementation details
