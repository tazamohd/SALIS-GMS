...
router.get('/sms/campaigns', isAuthenticated, requireRole(['ADMIN', 'MANAGER']), (_req, res) => {
...
router.post('/sms/campaigns', isAuthenticated, requireRole(['ADMIN', 'MANAGER']), (req, res) => {
...
router.get('/sms/campaigns/:id', isAuthenticated, requireRole(['ADMIN', 'MANAGER']), (req, res) => {
...
router.post('/sms/campaigns/:id/send', isAuthenticated, requireRole(['ADMIN', 'MANAGER']), (req, res) => {
  const campaign = campaigns.find(c => c.id === req.params.id);
  if (!campaign) {
    return res.status(404).json({ error: 'Campaign not found' });
  }
...
router.get('/sms/templates', isAuthenticated, requireRole(['ADMIN', 'MANAGER']), (_req, res) => {
  res.json({ templates });
});

// GET /api/sms/stats - Overall SMS stats
router.get('/sms/stats', isAuthenticated, requireRole(['ADMIN', 'MANAGER']), (_req, res) => {
