...
router.get('/ai/parts-recommendations', isAuthenticated, requireRole(['ADMIN', 'MANAGER', 'ADVISOR']), async (req, res) => {
...
router.post('/ai/parts-recommendations', isAuthenticated, requireRole(['ADMIN', 'MANAGER', 'ADVISOR']), validate(partsRecommendationSchema), async (req, res) => {
  const garageId = (req as any).user?.garageId || '1';
  try {
