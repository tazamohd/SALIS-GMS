...
describe("Fleet Accounts (DB-backed)", () => {
  it("POST /api/fleet/accounts creates and GET lists it", async () => {
    const create = await agent.post("/api/fleet/accounts").send({
    ...
    const list = await agent.get("/api/fleet/accounts");
  ...
  it("POST /api/fleet/accounts rejects missing companyName", async () => {
    const res = await agent.post("/api/fleet/accounts").send({ contactPerson: "x" });
