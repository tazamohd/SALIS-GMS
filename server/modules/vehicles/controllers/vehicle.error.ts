/**
 * Module error boundary (Phase E12) for the vehicle module. Domain errors render
...
export const vehicleErrorHandler: ErrorRequestHandler = (err, _req, res, _next) => {
  if (isDomainError(err)) {
    res.status(err.httpStatus).json({ message: err.message });
    return;
  }
  console.error('Unhandled error in vehicles module:', err);
