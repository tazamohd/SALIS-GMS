/**
 * Module error boundary (Phase E12) for the inventory module. Domain errors
...
export const inventoryErrorHandler: ErrorRequestHandler = (err, _req, res, _next) => {
  if (isDomainError(err)) {
    res.status(err.httpStatus).json({ message: err.message });
    return;
  }
  console.error('Unhandled error in inventory module:', err);
