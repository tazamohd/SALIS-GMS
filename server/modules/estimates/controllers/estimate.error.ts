/**
 * Module error boundary (Phase E12) for the estimate module. Domain errors
...
export const estimateErrorHandler: ErrorRequestHandler = (err, _req, res, _next) => {
  if (isDomainError(err)) {
    res.status(err.httpStatus).json({ message: err.message });
    return;
  }
  console.error('Unhandled error in estimates module:', err);
