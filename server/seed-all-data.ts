...
export async function seedAllData() {
  ...
  try {
    ...
    let garages: (typeof schema.garages.$inferSelect)[], branches: (typeof schema.branches.$inferSelect)[], saasPlans: (typeof schema.saasPlans.$inferSelect)[];
    ...
    let users: (typeof schema.users.$inferSelect)[], customers: (typeof schema.users.$inferSelect)[], technicians: (typeof schema.users.$inferSelect)[];
    
    if (existingUsers.length >= 50) {
      console.log('  ℹ️  Users already exist, skipping user creation...');
      users = existingUsers;
      customers = users.filter((u: typeof schema.users.$inferSelect) => u.userType === 'customer');
      technicians = users.filter((u: typeof schema.users.$inferSelect) => u.userType === 'technician');
    } else {
      users = [...existingUsers];
    ...
    let vehicles: (typeof schema.vehicles.$inferSelect)[];
    ...
    let jobCards: (typeof schema.jobCards.$inferSelect)[];
