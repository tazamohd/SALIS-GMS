...
export async function createAutoReorderRule(data: {
  garageId: string;
  partId: string;
  reorderPoint?: number;
  minQuantity?: number;
  ...
  try {
    const reorderPoint = data.reorderPoint ?? data.minQuantity;
    if (!reorderPoint) {
      throw new Error('Reorder point is required');
    }

    const [rule] = await db
      .insert(autoReorderRules)
      .values({
        garageId: data.garageId,
        partId: data.partId,
        reorderPoint,
        reorderQuantity: data.reorderQuantity,
        preferredSupplier: data.preferredSupplier ?? data.preferredSupplierId,
        maxPrice: data.maxPrice !== undefined ? data.maxPrice.toString() : undefined,
      })
      .returning();
