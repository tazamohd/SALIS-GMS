  ...
  res.on('finish', async () => {
    try {
      ...
      if (statusCode >= 200 && statusCode < 300) {
        ...
        // Platform-admin actions are cross-tenant. Log with NULL garageId
        // and include admin actor for traceability.
        if (resourceType === 'platform_admin') {
          const platformAdminLog: InsertAuditLog = {
            garageId: null,
            userId: user.id,
          ...
          await storage.createAuditLog(platformAdminLog);
          return;
        }
