...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Smart Search API (/api/search) is served by the modular, tenant-scoped
  // `server/routes/misc.routes.ts` (mounted ahead of this monolith in routes/index.ts).
  ...
  */
  ...
  // Service template CRUD (/api/service-templates*) is served by the modular,
  // tenant-scoped server/routes/service-templates.routes.ts (mounted ahead of this
  ...
  */
  ...
  app.get('/api/global-search', isAuthenticated, async (req: any, res) => {
    try {
      const { query, modules } = req.query;
      // Always scope to the caller's garage; never trust a client-supplied garageId
      // (prevents cross-tenant search). Fall back to the query param only when the
      // session has no garage assigned (e.g. platform-level accounts).
  ...
  // Saved filter presets + export jobs migrated to server/routes/filter-presets.routes.ts
  // and server/routes/export-jobs.routes.ts (garage + user scoped; export jobs are
  // personal). Mounted ahead of the monolith. Old handlers read garageId from the query
  // and had no per-garage/per-user ownership checks on update/delete/read — removed.
  ...
  */
  ...
  // Service bays + occupancy sessions migrated to server/routes/service-bays.routes.ts
  // (tenant-scoped; bay creation is ADMIN/MANAGER, status/sessions authenticated).
  // Mounted ahead of the monolith. Old handlers read garageId from the query and had
  // no per-garage ownership checks on status/session mutations — removed.
  ...
  */
  ...
  // AR instructions/sessions/devices migrated to server/routes/ar.routes.ts
  // (tenant-scoped + Zod; instruction mutations are ADMIN/MANAGER), mounted ahead
  // of the monolith. Old handlers read garageId from the query and lacked ownership
  // checks — removed.
  ...
  */
  ...
  app.get('/api/platform-admin/stats', isAuthenticated, async (req: any, res) => {
    try {
  ...
  app.get('/api/platform-admin/garages', isAuthenticated, async (req: any, res) => {
    try {
      const result = await db.execute(sql`
  ...
  app.post('/api/platform-admin/garages', isAuthenticated, async (req: any, res) => {
    try {
  ...
  app.patch('/api/platform-admin/garages/:id/status', isAuthenticated, async (req: any, res) => {
    try {
      const { status } = req.body;
  ...
  app.post('/api/platform-admin/suppliers', isAuthenticated, async (req: any, res) => {
    try {
  ...
  app.post('/api/platform-admin/stores', isAuthenticated, async (req: any, res) => {
    try {
  ...
  app.get('/api/platform-admin/support-tickets', isAuthenticated, async (req: any, res) => {
    try {
      const tickets = [
  ...
  app.patch('/api/platform-admin/support-tickets/:id', isAuthenticated, async (req: any, res) => {
    try {
  ...
  app.get('/api/platform-admin/system-health', isAuthenticated, async (req: any, res) => {
    try {
      res.json({
        apiLatency: 87,
        dbConnections: 124,
