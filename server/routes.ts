...
// Zod validation-error sanitizers now live in a shared util so the monolith and
// the extracted modules format validation responses identically (Phase E9).
import { sanitizeZodError, sanitizeArrayValidationErrors } from './utils/validation-errors';
...
// In production, this MUST be set; webhook returns 503 if missing.
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get('/api/technician-profiles/:userId', isAuthenticated, requireResourceOwnership({ table: 'users', idParam: 'userId' }), async (req, res) => {
    try {
      ...
      if (currentUserId !== userId && currentUserRole !== 'ADMIN') {
        ...
        const requesterGarageId = currentUser?.garageId || currentUser?.garage_id;
        const targetGarageId = (targetUser as any).garageId || (targetUser as any).garage_id;
        if (!requesterGarageId || requesterGarageId !== targetGarageId) {
          return res.status(403).json({ message: "Access denied: not in same garage" });
        }
      }
  ...
  app.patch('/api/technician-profiles/:userId', isAuthenticated, requireResourceOwnership({ table: 'users', idParam: 'userId' }), async (req, res) => {
    try {
      ...
      const validated = validatePatchBody(req, res, updateTechnicianProfileSchema);
      if (!validated.ok) return;
      const profile = await storage.updateTechnicianProfile(userId, validated.data as any);
      res.json(profile);
    } catch (error) {
  ...
  // Call Center Module (queues, members, sessions, notes, recordings,
  // disposition codes, agent performance) migrated to server/modules/call-center (Phase E).

  // Purchase order/item/task WRITE handlers migrated to server/modules/procurement (Phase E).

  // Purchase Agent - Quotation Management Routes
  app.post('/api/quotation-requests', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user?.id || 'default-user';
      const request = await storage.createQuotationRequest({ ...req.body, createdBy: userId } as any);
  ...
  app.post('/api/supplier-quotations', isAuthenticated, async (req, res) => {
    try {
      const { items, ...quotationData } = req.body;
      const quotation = await storage.createSupplierQuotation(quotationData);
  ...
  // Purchase Agent - Payment Tracking Routes
  app.post('/api/supplier-payments', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user?.id || 'default-user';
      const payment = await storage.createSupplierPayment({ ...req.body, createdBy: userId } as any);
  ...
  // Purchase Agent - Delivery Tracking Routes
  app.post('/api/deliveries', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user?.id || 'default-user';
      const { items, timeline: timelineEvents, liveStatus, ...deliveryData } = req.body;
      const delivery = await storage.createDelivery({ ...deliveryData, createdBy: userId } as any);
      if (items && Array.isArray(items)) {
        for (const item of items) {
          await storage.createDeliveryItem({ ...item, deliveryId: delivery.id });
        }
      }
      if (timelineEvents && Array.isArray(timelineEvents)) {
        for (const event of timelineEvents) {
          await storage.createDeliveryTimelineEvent({ ...event, deliveryId: delivery.id });
        }
      }
      if (liveStatus) {
        await storage.createLiveDeliveryStatus({ ...liveStatus, deliveryId: delivery.id });
      }
  ...
  app.patch('/api/deliveries/:id/live', isAuthenticated, requireResourceOwnership({ table: 'deliveries' }), async (req, res) => {
    try {
      ...
      if (!existing) {
        const liveStatus = await storage.createLiveDeliveryStatus({ ...req.body, deliveryId: id });
        return res.status(201).json(liveStatus);
      }
      const liveStatus = await storage.updateLiveDeliveryStatus(existing.id, req.body);
      res.json(liveStatus);
    } catch (error) {
      console.error("Error updating live delivery status:", error);
      res.status(500).json({ message: "Failed to update live delivery status" });
    }
  });

  // Module 12: Invoice & Billing — migrated to server/modules/invoices (Phase E).
  ...
  // Module 12b: Payments — migrated to server/modules/payments (Phase E).

  // Estimates & Quotes — migrated to server/modules/estimates (Phase E).
  ...
  app.post('/api/stripe/webhook', async (req, res) => {
    ...
    if (!STRIPE_WEBHOOK_SECRET) {
      console.error("CRITICAL: STRIPE_WEBHOOK_SECRET is not set. Refusing to process webhook to prevent forged events.");
      return res.status(503).json({
        message: "Webhook signing secret not configured. Set STRIPE_WEBHOOK_SECRET in environment."
      });
    }
    ...
    // express.json() populates req.body; the raw body is in req.rawBody
    // (set by the express.json verify hook below).
    const rawBody = (req as any).rawBody;
    if (!rawBody) {
      return res.status(400).json({
        message: "Webhook signature verification requires raw body. Server misconfigured."
      });
    }
  ...
  // Stock Alerts — migrated to server/modules/inventory (Phase E).

  // Reorder Settings + Pricing History — migrated to server/modules/procurement (Phase E).

  // Inventory Audit Trail + Transfers — migrated to server/modules/inventory (Phase E).
  ...
  // Maintenance Prediction Routes — extracted to server/modules/ai
  ...
  // AI OCR Document Routes — extracted to server/modules/ai
  // (GET /api/ai/ocr-documents, POST /api/ai/ocr-documents/upload,
  //  GET/PATCH /api/ai/ocr-documents/:id).

  // Phase 2: Advanced Analytics Routes
  // Analytics handlers (/api/analytics/*) migrated to server/modules/analytics (Phase E).
  ...
  app.post('/api/stripe/create-payment-intent', isAuthenticated, async (req: any, res) => {
    try {
      const validated = validatePatchBody(req, res, createPaymentIntentSchema);
      if (!validated.ok) return;

      const result = await phase3Service.createPaymentIntent(
  ...
  app.post('/api/stripe/refund', isAuthenticated, requireRole(['ADMIN', 'MANAGER', 'ACCOUNTANT']), async (req: any, res) => {
    try {
      ...
      const result = await phase3Service.processRefund(
        validated.data.paymentIntentId,
        validated.data.amount
      );
      res.json(result);
    } catch (error: any) {
  ...
  // Rate limiter for customer portal login (5 attempts per 15 min per IP) - brute force protection
  const customerPortalLoginLimiter = rateLimit({
    windowMs: 15 * 60 * 1000,
    max: 5,
  ...
  app.post('/api/customer-portal/login', customerPortalLoginLimiter, async (req, res) => {
    try {
  ...
  // Fleet Groups
  // Fleet-management groups/vehicles/contracts handlers migrated to server/modules/fleet-management (Phase E).
  ...
  // Fleet Pricing Tiers
  // Fleet-management pricing-tiers/maintenance-schedules handlers migrated to server/modules/fleet-management (Phase E).
  ...
  app.get("/api/environmental-compliance/records", isAuthenticated, async (req, res) => {
    // TODO: Replace with real DB query (currently stub)
    res.json([
  ...
  app.post("/api/environmental-compliance/records", isAuthenticated, async (req, res) => {
    try {
      ...
      const validated = validatePatchBody(req, res, createEnvironmentalRecordSchema);
      if (!validated.ok) return;

      // TODO: Replace stub with real DB insert when storage method exists
      // For now, echo the validated data with assigned id and timestamp
      res.status(201).json({
        id: `env-${Date.now()}`,
        garageId,
        ...validated.data,
      ...
      res.status(500).json({ message: "Failed to create environmental record" });
    }
  });
  ...
  // (dead-duplicate GET /api/analytics/custom-reports removed — owned by server/modules/analytics, Phase E.)
  ...
  app.post('/api/safety/incidents', isAuthenticated, async (req: any, res) => {
    try {
      ...
      const validated = validatePatchBody(req, res, createSafetyIncidentSchema);
      if (!validated.ok) return;
      ...
      const incidentData = {
        ...
        injuredPerson: validated.data.injuredPerson,
        witnessNames: validated.data.witnessNames,
        reportedBy: validated.data.reportedBy,
        immediateAction: validated.data.immediateAction,
        photos: validated.data.photos,
        incidentNumber: validated.data.incidentNumber,
        oshaRecordable: validated.data.oshaRecordable,
        medicalTreatment: validated.data.medicalTreatment,
        lostWorkDays: validated.data.lostWorkDays,
      };
  ...
  // Fleet-tracking handlers (locations, geofences, geofence-events, routes) migrated to server/modules/fleet-tracking (Phase E).
  ...
  // ==========================================
  // DYNAMIC PRICING SUGGESTIONS MODULE
  // ==========================================

  // Get market pricing data
  app.get('/api/dynamic-pricing/market-data', isAuthenticated, async (req: any, res) => {
    try {
  ...
  // Create market pricing data
  app.post('/api/dynamic-pricing/market-data', isAuthenticated, async (req: any, res) => {
    try {
  ...
  // Get vehicle pricing factors
  app.get('/api/dynamic-pricing/vehicle-factors', isAuthenticated, async (req: any, res) => {
    try {
  ...
  // Create vehicle pricing factor
  app.post('/api/dynamic-pricing/vehicle-factors', isAuthenticated, async (req: any, res) => {
    try {
  ...
  app.get('/api/dynamic-pricing/suggestions', isAuthenticated, async (req: any, res) => {
    try {
      const garageId = req.user?.garageId || req.query.garageId;
      if (!garageId) {
        return res.status(400).json({ message: "Garage ID is required" });
      }
      const { vehicleId, status } = req.query;
      const data = await storage.getDynamicPricingSuggestions(garageId, { 
  ...
  app.post('/api/dynamic-pricing/suggestions', isAuthenticated, async (req: any, res) => {
    try {
      const garageId = req.user?.garageId || req.body.garageId;
  ...
  app.post('/api/dynamic-pricing/calculate', isAuthenticated, async (req: any, res) => {
    try {
  ...
  // Get available service types for pricing
  app.get('/api/dynamic-pricing/service-types', isAuthenticated, async (req: any, res) => {
    try {
      const serviceTypes = [
  ...
  // Get vehicle classes
  app.get('/api/dynamic-pricing/vehicle-classes', isAuthenticated, async (req: any, res) => {
    try {
