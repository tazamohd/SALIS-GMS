...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Purchase Agent - Task Inbox Routes
  app.post('/api/purchase-tasks', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user?.id || 'default-user';
      const { parts, ...taskData } = req.body;
      const task = await storage.createPurchaseTask({ ...taskData, assignedTo: userId } as any);
      if (parts && Array.isArray(parts)) {
        for (const part of parts) {
          await storage.createPurchaseTaskPart({ ...part, taskId: task.id });
        }
      }
      res.status(201).json(task);
    } catch (error) {
      console.error("Error creating purchase task:", error);
      res.status(500).json({ message: "Failed to create purchase task" });
    }
  });
  ...
  app.patch('/api/purchase-tasks/:id', isAuthenticated, requireResourceOwnership({ table: 'purchase_tasks' }), async (req, res) => {
    try {
      ...
      res.status(500).json({ message: "Failed to update purchase task" });
    }
  });
